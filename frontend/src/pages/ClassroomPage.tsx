...
const ALL_COUNTRIES = Object.values(EMBEDDED_SDG_DATA).sort((a, b) =>
...
function buildIndicatorContext(
  ...
  if (goalScore !== undefined) {
    indicators.push({
      label: `SDG ${sdgGoal} Overall Score`,
      value: goalScore.toFixed(1) + "/100",
      status: goalScore >= 70 ? "good" : goalScore >= 50 ? "moderate" : "poor",
    });
  }
...
export default function ClassroomPage() {
  ...
  const indicators = useMemo(
    () => buildIndicatorContext(country, config.sdg_goal),
    [country, config.sdg_goal],
  );
  ...
  const topics = parseTopics(topicStream.text || ""); // Fixed: use text instead of output
      ...
      <div className="max-w-5xl mx-auto w-full p-6 space-y-5">
        ...
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">
            Real SDR 2025 country data → AI-generated SDG project topics for
            your class
          </p>
        </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              {goalInfo && (
                ...
                >
                  <span className="font-bold">{goalInfo.title}</span>{" "}
                  {/* Fixed: use goalInfo.title */}
                </div>
              )}
            </div>
            ...
            <div>
              <label className="text-slate-400 text-xs mb-2 block font-medium">
                Country{" "}
                <span className="text-slate-600">(uses real SDR data)</span>
              </label>
              <div className="relative">
                ...
                >
                  ...
                  <ChevronDown
                    size={13}
                    className={`text-slate-400 transition-transform ${showCountryPicker ? "rotate-180" : ""}`}
                  />
                </button>
                {showCountryPicker && (
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-3">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-blue-400 text-xs font-bold">
                    Real SDR 2025 Data → {country?.name}
                  </span>
                </div>
                <div className="space-y-1">
                  {indicators.map((ind, i) => (
                    ...
                    >
                      ...
                      <span
                        className={`font-mono font-bold flex-shrink-0 ${ind.status === "good" ? "text-green-400" : ind.status === "moderate" ? "text-yellow-400" : "text-red-400"}`}
                      >
                        {ind.value}
                      </span>
                    </div>
                ...
                <p className="text-slate-600 text-[10px] mt-2 flex items-center gap-1">
                  <Info size={9} />
                  AI will generate topics based on these real indicators
                </p>
              </div>
            )}
            ...
            <button
              ...
              style={{
                background: topicStream.loading ? "#374151" : goalColor,
              }}
            >
              {topicStream.loading ? (
                <>
              ...
              topics.length > 0 ? (
                <div className="space-y-3">
                  {topics.map((t: any, i: number) => (
                    <div
                      key={i}
                      className="bg-slate-900 border border-slate-800 rounded-2xl p-4 hover:border-slate-600 transition-all"
                    >
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4">
                  <StreamingOutput
                    text={topicStream.text}
                    loading={topicStream.loading}
                    error={topicStream.error}
                  />{" "}
                  {/* Fixed: added error prop, use text */}
                </div>
              )
            ) : (
              <div className="h-full min-h-[300px] border border-slate-800 border-dashed rounded-2xl flex flex-col items-center justify-center text-center p-8">
                ...
                {indicators.length > 0 && (
                  <p className="text-green-400 text-xs mt-2">
                    ✓ {indicators.length} real data points loaded
                  </p>
                )}
              </div>
            )}
