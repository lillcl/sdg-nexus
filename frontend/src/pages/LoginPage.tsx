...
export default function LoginPage() {
  const [form, setForm] = useState({
    email: '', password: '', mode: 'login' as 'login' | 'register',
    username: '', full_name: ''
  });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setAuth(meR.data, r.data.access_token, r.data.refresh_token);
      nav('/dashboard');
    } catch (e: unknown) {
        ...
        <div className="text-center mb-8">
          <div className="w-12 h-12 rounded-2xl overflow-hidden grid grid-cols-4 gap-[1px] p-[2px] bg-slate-800 mx-auto mb-4">
            {['#E5243B','#DDA63A','#4C9F38','#C5192D','#FF3A21','#26BDE2','#FCC30B','#A21942',
             ...
             ].map((c,i)=><div key={i} className="rounded-[2px]" style={{background:c}}/>)}
          </div>
          <h1 className="font-bold text-white text-2xl">SDG Nexus</h1>
          <p className="text-slate-500 text-sm mt-1">Interactive SDG Learning Platform</p>
        </div>

        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4">
          <div className="flex bg-slate-800 rounded-xl p-1">
            {(['login', 'register'] as const).map(m => (
              <button key={m} onClick={() => { setForm(p => ({ ...p, mode: m })); setError(''); setSuccess(''); }}
          ...
          {error && <p className="text-red-400 text-xs bg-red-950/30 border border-red-900/50 rounded-lg px-3 py-2">{error}</p>}
          {success && <p className="text-green-400 text-xs bg-green-950/30 border border-green-900/50 rounded-lg px-3 py-2">{success}</p>}
          ...
          <button onClick={handleSubmit} disabled={loading}
            ...
            {loading ? 'Please wait...' : form.mode === 'login' ? 'Sign In' : 'Create Account'}
          </button>

          <p className="text-center text-xs text-slate-500">
            Or <button onClick={() => nav('/')} className="text-blue-400 hover:underline">explore map without login →</button>
          </p>
        </div>
      </div>
    </div>
